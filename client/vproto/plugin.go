...
type plugin struct {
	...
	conn   stdnet.Conn       // plugin connection
	mux    multiplex.Mux     // connection multiplexer
	...
	server *server           // NRI server for this plugin
}
...
func (p *plugin) configure(ctx context.Context, config string) error {
	...
	err = p.server.start()
	if err != nil {
		log.Errorf("failed to start runtime ttRPC server for plugin %s: %v",
			p.id, err)
	}
